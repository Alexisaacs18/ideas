...
async function getEncryptionKey(userId, env) {
  ...
  const masterKey = env.ENCRYPTION_MASTER_KEY || env.ENCRYPTION_KEY;
  if (!masterKey) {
    throw new Error('ENCRYPTION_MASTER_KEY or ENCRYPTION_KEY secret not configured. Please set it in Cloudflare dashboard.');
  }
